...
class SignalingRoomClient {
    private lazy var manager = SocketManager(socketURL: Config.room.signalingServerUrl, config: [.log(true), .compress])
    ...
    private func setupListener() {
        ...
        socket.on("message") { [unowned self] data, _ in
        ...
        socket.on("user disconnected") { [unowned self] _, _ in
        ...
        socket.on("from_server") { data, _ in
