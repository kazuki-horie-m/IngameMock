...
//        RPBroadcastActivityViewController.load(handler: { [unowned self] vc, error in
        RPBroadcastActivityViewController.load(withPreferredExtension: "com.tctc.rinrin.StreamingDemoUpload2") { [unowned self] vc, error in
            if let vc = vc {
                vc.delegate = self
...
//        })
    ...
    private func finish() {
        broadCastController?.finishBroadcast(handler: { err in
            print("finished")
        })
    }
